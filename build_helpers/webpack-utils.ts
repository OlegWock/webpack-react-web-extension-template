...
type CacheGroups = Exclude<Exclude<Exclude<webpack.Configuration["optimization"], undefined>["splitChunks"], undefined | false>["cacheGroups"], undefined>;
// eslint-disable-next-line @typescript-eslint/ban-types
type CacheGroup = Exclude<CacheGroups[keyof CacheGroups], string | false | Function | RegExp>; 
// eslint-disable-next-line @typescript-eslint/ban-types
type BetterCacheGroupTest = Exclude<CacheGroup["test"], Function> | ((module: webpack.Module, { chunkGraph, moduleGraph }: { chunkGraph: webpack.ChunkGraph, moduleGraph: webpack.ModuleGraph }) => boolean);
...
export const scriptName = (filename: string) => filename.replace('.tsx', '').replace('.jsx', '').replace('.ts', '').replace('.js', '');
...
export const pathRelatedToExtRoot = <T extends keyof ReturnType<typeof createPathsObject>["dist"]>(paths: ReturnType<typeof createPathsObject>, prop: T): string => {
    return `/` + (paths.dist[prop].startsWith(paths.dist.base) ? paths.dist[prop].substring(paths.dist.base.length + 1) : paths.dist[prop]);
};
